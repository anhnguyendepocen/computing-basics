...
This work is a compilation of opinions, often in the form of suggestions  
phrased like, "you should". Additional explanatory material has been added 
...
accumulating computing knowledge over the years.

As we do not typically cite individual contributors or reference their specific 
...
authored by trusted and proven authorities.
...
CPUs and GPUs get hot and so are usually mated with a heat sink and a fan, 
all clamped together in one unit. The hotter the chip, the greater the need 
for cooling. Some CPUs offer http://en.wikipedia.org/wiki/Frequency_scaling[frequency scaling] 
which allows the chip to speed up as needed. As it speeds up, it heats up, and 
...
and the only recourse is to physically shut down the computer and restart it.
...
http://en.wikipedia.org/wiki/Random-access_memory[random access memory
(RAM)]. _Random access_ means you can, in principle, access any particular block of
...
_storage_. Files are stored in folders, or more generally, "storage". 
...
storage (hard disk) is full -- you have exceeded its capacity or your 
