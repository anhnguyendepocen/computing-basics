...
* *_Storage_* = drives = _long term_ = slower = "file storage" = _delete_ stuff to free it up
...
You should now see that the causes and remedies of memory and storage errors
are entirely separate and help to clarify the differences between the
two. 

However, now that we are clear on the distinction between memory
and storage, we have to mention the one feature, just to confuse you, called
