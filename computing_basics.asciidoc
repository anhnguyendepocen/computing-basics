...
http://en.wikipedia.org/wiki/Peripheral[peripherals]. The computer,
whether it is a http://en.wikipedia.org/wiki/Desktop_computer[desktop]
or http://en.wikipedia.org/wiki/Laptop[laptop], contains the "brains" --
...
The CPU, as the "brains" of the computer, is where the information is
...
cores in a CPU. A computer may also have a
...
connector. USB ports can vary in speed, so if speed matters, check your
...
primary storage], is the temporary "short term memory" also called
...
memory is lost. Memory is used to store data and applications that the
...
http://en.wikipedia.org/wiki/Random-access_memory[random access memory
(RAM)]. _Random access_ means you can access any particular block of
memory you want _at random_ -- without having to sequentially search for
it. RAM is installed in your computer packaged as
http://en.wikipedia.org/wiki/Integrated_circuit[chips], often on a
...
_storage_. Files are stored in "storage". Examples of storage devices
...
Other examples of
http://en.wikipedia.org/wiki/Computer_data_storage#Off-line_storage[off-
line storage] are http://en.wikipedia.org/wiki/Flash_memory[flash
...
freeing it. You can free memory by closing applications, files, and
"browser tabs" which don't need to be open right now. Releasing memory
can make a huge impact in computer performance.

Alternatively, if you receive a "disk full" error, this means that your
hard disk (storage) is full. Sometimes the disk fills when an
application is buggy and writes way more data to the disk than it
should. More often, people will their hard drives with files over time,
until eventually there is no more room. When this happens, software can
no longer write to the storage and will usually either halt with an
error message or keep trying causing strange delays. People usually free
up storage space by removing old or temporary files no longer needed, or
are backed up somewhere else. You may also use a software tool called a
_disk cleaning utility_ or similar to automate this task somewhat.

=== Virtual Memory ===

So, you see that the causes and remedies of memory and storage errors
are entirely separate and help to clarify the differences between the
two. However, now that we are clear on the distinction between memory
...
an "overflow" area for volatile memory. If you fill up your memory, then
...
you can imagine, this procedure also carries more risk to your data. For
this reason, people are more likely to use an external drive if they
need more storage, or even use an online storage service.
